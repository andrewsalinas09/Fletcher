...
## Q-17 · Preset-history tree — **v1 shipped 2026-08-23; residuals open**
Shipped: in-session undo *graph* — nodes are completed gestures (mouse drop / Enter / wheel settled 250 ms, per user spec), Ctrl+Z/Y walk the tree, post-undo edits fork branches, jumpable tree UI with branch markers. Residual open: **persistence** (survive restarts, per-preset journals on disk), the full visual graph view (its own room, per the product principle), and wiring "blind A/B any two tree nodes" into the Lab.
...
## Q-20 · Pop-out windows with synced state — **Open (leaning yes)**
